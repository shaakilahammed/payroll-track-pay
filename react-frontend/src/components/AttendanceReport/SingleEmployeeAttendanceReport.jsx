...
import AttendaceReportSingleEmployeeTable from '../Table/AttendanceReport/AttendaceReportSingleEmployeeTable';
import { useReactToPrint } from 'react-to-print';
import { Link } from 'react-router-dom';

const SingleEmployeeAttendanceReport = () => {
      ...
      {!isLoading && !isError && !isDateChange && isSuccess && (
        <>
          <div className={styles.header}>
            <h3>
              Attendance Report ({`${values.start_date} to ${values.end_date}`})
            </h3>
            <Link to="#" className={styles.btn} onClick={handlePrint}>
              Print
            </Link>
          </div>
          <AttendaceReportSingleEmployeeTable
